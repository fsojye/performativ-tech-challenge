...
from models.decimals import NAN, ONE, ZERO
...
class PositionCalculator:
    ...
    def _calculate_open_value(self) -> Decimal:
    ...
    def _calculate_close_value(self) -> Decimal:
    ...
    def calculate_price_local(self, date_index: DatetimeIndex) -> Series[Decimal]:
        price = Series(ZERO, index=date_index).where(self._day_is_pre_open(date_index), self._prices["price"])
        return price.mask(self._day_is_pre_open(date_index), ZERO)

    def calculate_price(self, date_index: DatetimeIndex, price_local: Series[Decimal]) -> Series[Decimal]:
        price_local = price_local.reindex(date_index)
        fx_rates = self._fx_rates["rate"].reindex(date_index)

        return price_local * fx_rates

    def calculate_is_open(self, date_index: DatetimeIndex) -> Series[Decimal]:
        is_open = Series(ZERO, index=date_index).mask(self._day_is_within_open(date_index), ONE)
        return is_open

    def calculate_quantity(self, date_index: DatetimeIndex, is_open: Series[Decimal]) -> Series[Decimal]:
        is_open = is_open.reindex(date_index)
        return is_open * self._position.quantity

    def calculate_value_local(
        self, date_index: DatetimeIndex, price_local: Series[Decimal], quantity: Series[Decimal]
    ) -> Series[Decimal]:
        price_local = price_local.reindex(date_index)
        quantity = quantity.reindex(date_index)
        return price_local * quantity

    def calculate_value(self, date_index: DatetimeIndex, value_local: Series[Decimal]) -> Series[Decimal]:
        value_local = value_local.reindex(date_index)
        fx_rates = self._fx_rates["rate"].reindex(date_index)
        return value_local * fx_rates

    def calculate_value_start(self, date_index: DatetimeIndex, value: Series[Decimal]) -> Series[Decimal]:
        open_value = self._calculate_open_value()
        value_start = value.reindex(date_index).shift(1, fill_value=ZERO)
    ...
    def calculate_value_end(self, date_index: DatetimeIndex, value: Series[Decimal]) -> Series[Decimal]:
    ...
    def calculate_return_per_period(
        self, date_index: DatetimeIndex, value_end: Series[Decimal], value_start: Series[Decimal]
    ) -> Series[Decimal]:
        return_per_period = Series(ZERO, index=date_index).mask(
    ...
    def calculate_return_per_period_percentage(
        self, date_index: DatetimeIndex, value_start: Series[Decimal], return_per_period: Series[Decimal]
    ) -> Series[Decimal]:
        return_per_period_percentage = Series(ZERO, index=date_index)
        mask = value_start != ZERO
        return_per_period_percentage[mask] = return_per_period[mask].combine(
            value_start[mask], lambda ret, start: ret / start if start != 0 else ZERO
        )
