...
class PositionCalculator:
    ...
    def _day_is_pre_close(self, close_date: Timestamp) -> NDArray:
        close_bound = close_date or (to_datetime(self._date_index[-1].date()) + Timedelta(days=1))
        return self._date_index < close_bound  # type: ignore

    def _day_is_within_open(self, open_date: Timestamp, close_date: Timestamp) -> NDArray:
    ...
    def _day_is_within_open_or_is_close(self, open_date: Timestamp, close_date: Timestamp) -> NDArray:
    ...
    def _price_local_calculate(self, dataframe: DataFrame, open_date: Timestamp) -> None:
        dataframe[PositionMetricFields.PRICE_LOCAL] = self._prices["price"].mask(self._day_is_pre_open(open_date), 0.0)
    ...
    def _open_price_target_calculate(self, dataframe: DataFrame, open_price: float) -> None:
        dataframe[PositionMetricFields.OPEN_PRICE_TARGET] = open_price * self._fx_rates["rate"]

    def _close_price_target_calculate(self, dataframe: DataFrame, close_price: float | None) -> None:
        dataframe[PositionMetricFields.CLOSE_PRICE_TARGET] = (
            close_price if close_price is not None else nan
        ) * self._fx_rates["rate"]

    def _is_open_calculate(self, dataframe: DataFrame, open_date: Timestamp, close_date: Timestamp) -> None:
        dataframe[PositionMetricFields.IS_OPEN] = self._day_is_within_open(open_date, close_date).astype(float)

    def _quantity_calculate(self, dataframe: DataFrame, quantity: float) -> None:
        dataframe[PositionMetricFields.QUANTITY] = dataframe[PositionMetricFields.IS_OPEN] * quantity

    def _value_local_calculate(self, dataframe: DataFrame) -> None:
        dataframe[PositionMetricFields.VALUE_LOCAL] = (
            dataframe[PositionMetricFields.PRICE_LOCAL] * dataframe[PositionMetricFields.QUANTITY]
        )
    ...
    def _open_value_target_calculate(self, dataframe: DataFrame, quantity: float, open_date: Timestamp) -> None:
    ...
    def _close_value_target_calculate(self, dataframe: DataFrame, quantity: float, close_date: Timestamp) -> None:
    ...
    def _value_end_target_calculate(self, dataframe: DataFrame, close_date: Timestamp) -> None:
        dataframe[PositionMetricFields.VALUE_END_TARGET] = dataframe[PositionMetricFields.VALUE_TARGET].mask(
            self._day_is_close(close_date),
            dataframe[PositionMetricFields.CLOSE_VALUE_TARGET],
        )

    def _value_start_target_calculate(self, dataframe: DataFrame, open_date: Timestamp) -> None:
    ...
    def _return_per_period_calculate(self, dataframe: DataFrame, open_date: Timestamp, close_date: Timestamp) -> None:
        dataframe[PositionMetricFields.RETURN_PER_PERIOD] = (
            dataframe[PositionMetricFields.VALUE_END_TARGET] - dataframe[PositionMetricFields.VALUE_START_TARGET]
    ...
    def _return_per_period_percentage_calculate(self, dataframe: DataFrame) -> None:
        dataframe[PositionMetricFields.RETURN_PER_PERIOD_PERCENTAGE] = (
            dataframe[PositionMetricFields.RETURN_PER_PERIOD] / dataframe[PositionMetricFields.VALUE_START_TARGET]
